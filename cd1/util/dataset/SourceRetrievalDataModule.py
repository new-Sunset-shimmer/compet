...
from util.dataset.BasicDatasetForBook import BasicDatasetForBook
from util.dataset.BasicDatasetForBookTest import BasicDatasetForBookTest
...
class LoadDataset(Dataset):
    ...
    def make_dataset(self, data, _config):
        ...
        for i in range(len(data)):
            ...
            # caption = data[i]['caption'] # dictonary key 이름 = caption
            # image_index.append(i)
            
            # text = self.tokenizer.encode(title, add_special_tokens=False)
            # text2 = self.tokenizer.encode(caption, add_special_tokens=False)
            
            # # padding
            # if len(text) + len(text2) <= self.seq_len - 3:
            #     text = [self.cls_token_id] + text + [self.sep_token_id] + text2 + [self.sep_token_id]
            #     pad_length = self.seq_len - (len(text) + len(text2))
            #     attention_mask = (len(text) * [1]) + (pad_length * [0]) + (len(text2) * [1]) + (pad_length * [0])
            ...
            text = self.tokenizer.encode_plus(text = title,text_pair=caption, add_special_tokens=True, padding='max_length', max_length= _config['input_seq_len'], return_attention_mask=True)
