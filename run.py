...
df = pd.read_csv('/home2/yangcw/bookdatas/train_data.csv')
df['label'].unique()
...
df2 = pd.read_csv('/home2/yangcw/bookdatas/caption_data.csv')
sentences1 = df2.caption.values
...
for id,sent in enumerate(sentences):
  input_ids.append(tokenizer.encode(text=sent,text_pair=sentences1[id],add_special_tokens=True,max_length=MAX_LEN,pad_to_max_length=True))
labels = df.label_enc.values
...
# input_idsi = [processor_image(PIL.Image.open("/home2/yangcw/bookdatas/train/"+f"{to_img[id]}/"+image), return_tensors="pt") for id,image in enumerate(images)]
# train_data = TensorDataset(train_inputs,train_masks,train_labels)
# train_sampler = RandomSampler(train_data)
# train_dataloader = DataLoader(train_data,sampler=train_sampler,batch_size=batch_size// accumulation)
# validation_data = TensorDataset(validation_inputs,validation_masks,validation_labels)
...
optimizer = AdamW(model.parameters(), lr=lr,eps=adam_epsilon,correct_bias=False,no_deprecation_warning=True,betas=(0.9,0.91))  # To reproduce BertAdam specific behavior set correct_bias=False
...
for _ in tqdm(range(epochs)):
  ...
  if eval_accuracy/nb_eval_steps>=maxer:
    ...
    torch.save(model,'e5-large-v2_run_model'+f'{q23}'+'.pt')
    print(q23,"saved")
    maxer = q23
    with open("/home2/yangcw/"+"e5-large-v2_run_model"+f"{q23}"+"txt", "w") as file:
        file.write("run: "+"batch_size:"+f"{batch_size}"+" accumulation:"+f"{accumulation}"+" SEED:"+f"{SEED}"+" lr:"+f"{lr}") 
  print(F'\n\tValidation Accuracy: {eval_accuracy/nb_eval_steps} old: {maxer}')
...
with torch.no_grad(), open('/home2/yangcw/run_submission'+'.csv', 'w') as fd:
