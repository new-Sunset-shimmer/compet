...
def main(_config):
    ...
    modelsr = _config["model_name"]
    os.makedirs(_config["log_dir"], exist_ok=True)
    checkpoint_callback = pl.callbacks.ModelCheckpoint(
        # save_top_k=3,
        verbose=True,
        monitor="val/acc",
        filename='epoch={epoch}-step={step}_model={modelsr}-val_acc={val/acc:.5f}',
        mode="max",
        save_last=True,
    ...
    logger = pl.loggers.TensorBoardLogger(
        _config["log_dir"],
        name=f'{exp_name}_seed{_config["seed"]}_model={_config["model_name"]}_from_{_config["load_path"].split("/")[-1][:-5]}',
    )
